...
// ユーザー文面の「date直下」は「data直下」の可能性があるため、
// data/recipe.csv を優先し、無ければ repo直下 recipe.csv を読む。
const CSV_CANDIDATES = ["data/recipe.csv", "recipe.csv"];
...
// 既存のフォームはデザイン維持用（CSV管理前提のため送信は抑止）
...
async function init() {
  try {
    // 候補パスを順に試し、最初に見つかったCSVを採用する。
    // 競合時の配置差分（data/ 配下 or 直下）を吸収するための実装。
    let csvText = "";
    let loaded = false;
    ...
    if (!loaded) {
      throw new Error("recipe.csv の読み込みに失敗しました。");
    }
