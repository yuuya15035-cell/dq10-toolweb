...
const HOME_FEATURE_DEFINITIONS = [
  ...
  { id: "crystal-simulator", tabId: "crystal-simulator", title: "結晶シミュレーター", icon: "◇" },
...
const CRYSTAL_SIMULATOR_TABS = Object.freeze([
  { id: "buy", label: "装備買って結晶化" },
  { id: "buy-alchemy", label: "装備買って錬金" },
  { id: "craft-alchemy", label: "装備作って錬金" },
]);
const CRYSTAL_SIMULATOR_DEFAULTS = Object.freeze({
  crystalUnitPrice: 3000,
  feeRate: 5,
  equipmentLevel: 42,
  star: 2,
  purchasePrice: 0,
  bazaarListingPrice: 0,
  materialCost: 0,
});
...
const CRYSTAL_EQUIPMENT_SERIES_SUFFIXES = Object.freeze([
  "からだ上",
  "からだ下",
  "コート上",
  "コート下",
...
let selectedCraftRecordItemName = "";
let craftRecordListDateKeyword = "";
let craftRecordListItemKeyword = "";
let selectedCraftRecordListSort = "date_desc";
let craftRecordCountHistory = [];
let craftRecordWorkStartTime = "";
...
let isCrystalMaterialBreakdownExpanded = false;
let isCrystalCraftMaterialCostManual = false;
let crystalSimulatorState = {
  ...
  bazaarListingPrice: CRYSTAL_SIMULATOR_DEFAULTS.bazaarListingPrice,
  materialCost: CRYSTAL_SIMULATOR_DEFAULTS.materialCost,
  alchemyRecipeId: "",
  alchemyEffectName: "",
  alchemyUnitCost: 0,
};
...
const craftRecordStar0SalesInput = getRequiredElementById("craftRecordStar0SalesInput");
const craftRecordStar1SalesInput = getRequiredElementById("craftRecordStar1SalesInput");
const craftRecordStar2SalesInput = getRequiredElementById("craftRecordStar2SalesInput");
const craftRecordStar3SalesInput = getRequiredElementById("craftRecordStar3SalesInput");
...
const craftRecordTodaySummaryWrap = getRequiredElementById("craftRecordTodaySummaryWrap");
const craftRecordPeriodSelect = getRequiredElementById("craftRecordPeriodSelect");
const craftRecordItemFilterSelect = getRequiredElementById("craftRecordItemFilterSelect");
const craftRecordDailySummaryWrap = getRequiredElementById("craftRecordDailySummaryWrap");
const craftRecordListWrap = getRequiredElementById("craftRecordListWrap");
const craftRecordListDateFilterInput = getRequiredElementById("craftRecordListDateFilterInput");
const craftRecordListItemFilterInput = getRequiredElementById("craftRecordListItemFilterInput");
const craftRecordListSortSelect = getRequiredElementById("craftRecordListSortSelect");
const craftRecordCountButtons = document.querySelectorAll("[data-craft-record-count-star]");
const craftRecordUndoCountButton = getRequiredElementById("craftRecordUndoCountButton");
const craftRecordResetCountButton = getRequiredElementById("craftRecordResetCountButton");
const craftRecordWorkStartButton = getRequiredElementById("craftRecordWorkStartButton");
...
const crystalSimulatorWrap = getRequiredElementById("crystalSimulatorWrap");
...
function formatCrystalGold(value) {
  return `${Math.round(Number.isFinite(value) ? value : 0).toLocaleString("ja-JP")}G`;
}

function formatCrystalPercent(value) {
...
function getStarCount(value) {
  ...
  if (!Number.isFinite(parsed)) return CRYSTAL_SIMULATOR_DEFAULTS.star;
  return Math.max(0, Math.min(3, parsed));
}

function getAlchemyCountFromStar(star) {
...
function isCrystalizableEquipmentRow({ craftsman = "", category = "", equipmentName = "" } = {}) {
  const normalizedCraftsman = String(craftsman || "").trim();
  const normalizedCategory = String(category || "").trim();
  const normalizedEquipmentName = String(equipmentName || "").trim();
  if (normalizedEquipmentName === "") return false;
  if (CRYSTAL_EQUIPMENT_EXCLUDED_CRAFTSMEN.has(normalizedCraftsman)) return false;
  if (CRYSTAL_EQUIPMENT_EXCLUDED_CATEGORIES.has(normalizedCategory)) return false;
  if (CRYSTAL_EQUIPMENT_ALLOWED_CRAFTSMEN.has(normalizedCraftsman)) return true;
  return false;
}

function getCrystalEquipmentSeriesName(equipmentName) {
  const normalizedName = String(equipmentName || "").trim();
  if (normalizedName === "") return "";
  const matchedSuffix = CRYSTAL_EQUIPMENT_SERIES_SUFFIXES.find((suffix) => normalizedName.endsWith(suffix));
  if (!matchedSuffix) return normalizedName;
  const seriesName = normalizedName.slice(0, -matchedSuffix.length).replace(/の$/, "").trim();
  return seriesName || normalizedName;
}
...
function extractEquipmentListFromRecipeCsv(sourceRows = []) {
  ...
  if (sourceRows.length > 0 && typeof sourceRows[0] === "string") {
    ...
    sourceRows.slice(1).forEach((line) => {
      ...
      const craftsman = craftsmanIndex >= 0 ? row[craftsmanIndex] : "";
      ...
      if (!isCrystalizableEquipmentRow({ craftsman, category, equipmentName })) return;
      ...
      if (!equipmentMap.has(key)) {
        equipmentMap.set(key, {
          ...
          name: equipmentName,
          equipmentLevel,
          series: getCrystalEquipmentSeriesName(equipmentName),
          craftsman,
          category,
        });
      }
    ...
    sourceRows.forEach((equipment) => {
      ...
      const craftsman = String(equipment?.craftsman || "").trim();
      const category = String(equipment?.category || "").trim();
      if (equipmentName === "" || equipmentLevel === null) return;
      if (!isCrystalizableEquipmentRow({ craftsman, category, equipmentName })) return;
      const key = makeCrystalEquipmentKey(equipmentName, equipmentLevel);
      if (!equipmentMap.has(key)) {
        equipmentMap.set(key, { key, name: equipmentName, equipmentLevel, series: getCrystalEquipmentSeriesName(equipmentName), craftsman, category });
      }
    });
  }
...
function getEquipmentSeriesByLevel(level) {
  ...
  filterEquipmentsByLevel(level).forEach((equipment) => {
    const series = String(equipment.series || equipment.name || "").trim();
    if (series !== "" && !seriesMap.has(series)) seriesMap.set(series, series);
  });
  return Array.from(seriesMap.values()).sort((a, b) => a.localeCompare(b, "ja"));
}

function filterEquipmentsByLevelAndSeries(level, seriesName) {
  const normalizedSeries = String(seriesName || "").trim();
  return filterEquipmentsByLevel(level)
    .filter((equipment) => String(equipment.series || equipment.name || "").trim() === normalizedSeries)
...
function onEquipmentLevelChange(level) {
  isCrystalCraftMaterialCostManual = false;
...
function applyCrystalCraftMaterialCostIfAvailable() {
  ...
  if (isCrystalCraftMaterialCostManual) return;
  updateCraftAlchemyCost();
}
...
function getCrystalEquipmentLevel(value) {
  const parsed = Math.floor(Number(String(value ?? "").replace(/,/g, "").trim()));
  return Number.isFinite(parsed) ? Math.max(1, parsed) : CRYSTAL_SIMULATOR_DEFAULTS.equipmentLevel;
}
...
function getCrystalAlchemyPoint(alchemyGrade = "beginner", alchemyResult = "success") {
  const grade = CRYSTAL_ALCHEMY_RESULT_POINTS[alchemyGrade] ? alchemyGrade : "beginner";
  const result = CRYSTAL_ALCHEMY_RESULT_POINTS[grade][alchemyResult] !== undefined ? alchemyResult : "success";
  return CRYSTAL_ALCHEMY_RESULT_POINTS[grade][result];
}
...
function getCrystalNetSale(amount) {
  const rate = CRYSTAL_SIMULATOR_FIXED_FEE_RATE / 100;
  return parseGoldInput(amount) * (1 - rate);
}

function getCrystalJudgement(profit) {
  if (profit > 0) return "黒字";
  if (profit < 0) return "赤字";
  return "トントン";
}

function getCrystalRecommendation(crystalProfit, bazaarProfit) {
  if (crystalProfit > bazaarProfit) return "結晶化";
  if (crystalProfit < bazaarProfit) return "バザー出品";
  return "どちらでも同じ";
}

function getCrystalMaterialPriceByName(materialName) {
  const normalizedName = normalizeMaterialNameKey(materialName);
  if (normalizedName === "") return 0;
  const bazaarRow = bazaarRowByMaterialName.get(normalizedName);
  const bazaarPrice = getPreferredBazaarUnitPrice(bazaarRow);
  if (Number.isFinite(bazaarPrice)) return bazaarPrice;
  const material = (state.materials || []).find((row) => normalizeMaterialNameKey(row?.name) === normalizedName);
  const materialPrice = Number(material?.price || 0);
  return Number.isFinite(materialPrice) ? materialPrice : 0;
}

function getRecipeByEquipmentName(equipmentName) {
  const normalizedName = String(equipmentName || "").trim();
  if (normalizedName === "") return null;
  const equipment = (state.equipments || []).find((entry) => String(entry?.name || "").trim() === normalizedName);
  if (!equipment) return null;
  const rows = (state.recipes || []).filter((row) => row.equipmentId === equipment.id);
  return rows.length > 0 ? { equipment, rows } : null;
}

function extractRecipeMaterials(recipe) {
  if (!recipe?.rows?.length) return [];
  const materialMap = new Map((state.materials || []).map((material) => [material.id, material]));
  ...
  recipe.rows.forEach((row) => {
    ...
    const current = groupedMaterials.get(materialName) || { materialName, quantity: 0 };
    current.quantity += quantity;
    groupedMaterials.set(materialName, current);
  });

  return Array.from(groupedMaterials.values()).sort((a, b) => a.materialName.localeCompare(b.materialName, "ja"));
}

function getMaterialPrice(materialName) {
  return getCrystalMaterialPriceByName(materialName);
}

function calculateCraftMaterialCost(equipmentName = crystalSimulatorState.equipmentName) {
  const recipe = getRecipeByEquipmentName(equipmentName);
  const materials = extractRecipeMaterials(recipe).map((material) => {
    const unitPrice = getMaterialPrice(material.materialName);
    const safeUnitPrice = Number.isFinite(unitPrice) ? Math.max(0, Math.round(unitPrice)) : 0;
    const subtotal = safeUnitPrice * material.quantity;
    return {
      ...material,
      unitPrice: safeUnitPrice,
      subtotal,
      isMissingPrice: safeUnitPrice <= 0,
    };
  });
  const totalCost = materials.reduce((sum, material) => sum + material.subtotal, 0);
  return {
    recipe,
    materials,
    totalCost,
    missingMaterials: materials.filter((material) => material.isMissingPrice).map((material) => material.materialName),
  };
}

function updateCraftAlchemyCost() {
  const cost = calculateCraftMaterialCost(crystalSimulatorState.equipmentName);
  crystalSimulatorState.materialCost = cost.totalCost;
  return cost;
}

function getCrystalUnitPriceFromBazaar() {
  const price = getCrystalMaterialPriceByName("汗と涙の結晶");
  return price > 0 ? price : CRYSTAL_SIMULATOR_DEFAULTS.crystalUnitPrice;
}

function getCrystalEquipmentBazaarPrice(equipmentName, star) {
  const normalizedName = String(equipmentName || "").trim();
  const normalizedStar = getStarCount(star);
  if (normalizedName === "" || normalizedStar === 0) return 0;
  const matchedRow = (bazaarEquipmentPrices || []).find((row) => String(row?.itemName || "").trim() === normalizedName && Number(row?.star) === normalizedStar);
  const price = Number(matchedRow?.todayPrice);
...
function applyCrystalAutoPrices(options = {}) {
  ...
  if (forceCrystal || parseGoldInput(crystalSimulatorState.crystalUnitPrice) <= 0 || crystalSimulatorState.crystalUnitPrice === CRYSTAL_SIMULATOR_DEFAULTS.crystalUnitPrice) {
    crystalSimulatorState.crystalUnitPrice = getCrystalUnitPriceFromBazaar();
  }
}

function loadAlchemyRecipes() {
  const equipmentById = new Map((state.equipments || []).map((equipment) => [equipment.id, equipment]));
  const recipeGroups = new Map();

  (state.recipes || []).forEach((row) => {
    const equipment = equipmentById.get(row.equipmentId);
    const craftsman = String(equipment?.craftsman || "").trim();
    const category = String(equipment?.category || "").trim();
    if ((craftsman !== "ツボ職人" && craftsman !== "ランプ職人") || category !== "初級") return;
    const group = recipeGroups.get(row.equipmentId) || {
      id: row.equipmentId,
      type: craftsman === "ツボ職人" ? "tsubo" : "lamp",
      typeLabel: craftsman === "ツボ職人" ? "ツボ初級" : "ランプ初級",
      effectName: String(equipment?.name || "").trim(),
      materials: [],
      unitCost: 0,
    };
    const material = (state.materials || []).find((entry) => entry.id === row.materialId);
    const materialName = String(material?.name || "").trim();
    const quantity = Math.max(0, Number(row.quantity || 0));
    const unitPrice = getCrystalMaterialPriceByName(materialName);
    const cost = unitPrice * quantity;
    group.materials.push({ materialName, quantity, unitPrice, cost });
    group.unitCost += cost;
    recipeGroups.set(row.equipmentId, group);
  });

  return Array.from(recipeGroups.values()).sort((a, b) => {
    if (a.unitCost !== b.unitCost) return a.unitCost - b.unitCost;
    return a.effectName.localeCompare(b.effectName, "ja");
  });
}

function getCheapestAlchemyRecipes(type, limit = 3) {
  return loadAlchemyRecipes().filter((recipe) => recipe.type === type).slice(0, limit);
}

function getSelectedAlchemyRecipe() {
  const recipes = loadAlchemyRecipes();
  let selected = recipes.find((recipe) => recipe.id === crystalSimulatorState.alchemyRecipeId);
  if (!selected) {
    selected = recipes[0] || null;
  }
  if (selected && !crystalSimulatorState.alchemyRecipeId) {
    crystalSimulatorState.alchemyRecipeId = selected.id;
    crystalSimulatorState.alchemyEffectName = selected.effectName;
    crystalSimulatorState.alchemyUnitCost = selected.unitCost;
  }
  return selected;
}
...
function calculateBuyCrystal() {
  const crystalCount = getCrystalCountForCurrentState();
  const crystalNetSales = getCrystalNetSale(crystalSimulatorState.crystalUnitPrice * crystalCount);
  const purchasePrice = parseGoldInput(crystalSimulatorState.purchasePrice);
  const profit = crystalNetSales - purchasePrice;
  return {
    crystalCount,
    crystalNetSales,
    purchasePrice,
    profit,
    profitRate: purchasePrice > 0 ? (profit / purchasePrice) * 100 : 0,
    judgement: getCrystalJudgement(profit),
  };
}

function calculateBuyAlchemy() {
  const recipe = getSelectedAlchemyRecipe();
  const star = getStarCount(crystalSimulatorState.star);
  const alchemyMaterialCost = (Number(recipe?.unitCost) || 0) * star;
  const totalCost = parseGoldInput(crystalSimulatorState.purchasePrice) + alchemyMaterialCost;
  const crystalCount = getCrystalCountForCurrentState();
  const crystalProfit = getCrystalNetSale(crystalSimulatorState.crystalUnitPrice * crystalCount) - totalCost;
  const bazaarProfit = getCrystalNetSale(crystalSimulatorState.bazaarListingPrice) - totalCost;
  return {
    recipe,
    totalCost,
    alchemyMaterialCost,
    ...
    crystalProfit,
    bazaarProfit,
    recommendation: getCrystalRecommendation(crystalProfit, bazaarProfit),
    difference: crystalProfit - bazaarProfit,
  };
}

function calculateCraftAlchemy() {
  const recipe = getSelectedAlchemyRecipe();
  const star = getStarCount(crystalSimulatorState.star);
  const productionCost = parseGoldInput(crystalSimulatorState.materialCost);
  const alchemyMaterialCost = (Number(recipe?.unitCost) || 0) * star;
  const totalCost = productionCost + alchemyMaterialCost;
  const crystalCount = getCrystalCountForCurrentState();
  const crystalProfit = getCrystalNetSale(crystalSimulatorState.crystalUnitPrice * crystalCount) - totalCost;
  const bazaarProfit = getCrystalNetSale(crystalSimulatorState.bazaarListingPrice) - totalCost;
  return {
    ...
    recommendation: getCrystalRecommendation(crystalProfit, bazaarProfit),
    difference: crystalProfit - bazaarProfit,
    profitRate: totalCost > 0 ? (Math.max(crystalProfit, bazaarProfit) / totalCost) * 100 : 0,
  };
}

function getCrystalStarOptions() {
  return [
    { value: 1, label: "★" },
    { value: 2, label: "★★" },
    { value: 3, label: "★★★" },
...
function renderCrystalMaterialBreakdown() {
  const breakdown = calculateCraftMaterialCost(crystalSimulatorState.equipmentName);
  const hasMaterials = breakdown.materials.length > 0;
  const missingText =
    breakdown.missingMaterials.length > 0
      ? `<p class="crystal-material-breakdown-warning">価格未取得素材あり: ${escapeHtml(breakdown.missingMaterials.join("、"))}</p>`
      : "";
  const toggleText = `${isCrystalMaterialBreakdownExpanded ? "▼" : "▶"} 素材内訳を見る`;
  return `
    <div class="crystal-material-breakdown">
      ${missingText}
      <button type="button" class="crystal-material-breakdown-toggle" data-crystal-material-breakdown-toggle aria-expanded="${isCrystalMaterialBreakdownExpanded ? "true" : "false"}">${toggleText}</button>
      ${
        isCrystalMaterialBreakdownExpanded
          ? `<div class="crystal-material-breakdown-list">
              ${
                hasMaterials
                  ? breakdown.materials
                      .map(
                        (material) => `
                          <article class="crystal-material-breakdown-item ${material.isMissingPrice ? "is-missing-price" : ""}">
                            <p class="crystal-material-breakdown-name">${escapeHtml(material.materialName)} × ${escapeHtml(material.quantity)}</p>
                            <p class="crystal-material-breakdown-price">
                              <span>${formatCrystalGold(material.unitPrice)}</span>
                              <span>小計 ${formatCrystalGold(material.subtotal)}</span>
                            </p>
                          </article>
                        `
                      )
                      .join("")
                  : `<p class="site-search-empty assist-search-empty">この装備の素材レシピが見つかりません。</p>`
              }
              <p class="crystal-material-breakdown-total">合計: <strong>${formatCrystalGold(breakdown.totalCost)}</strong></p>
            </div>`
          : ""
      }
    </div>
...
function renderCrystalStarField() {
  return `
    <label class="field crystal-field">
      <span>できのよさ</span>
      <select data-crystal-field="star">
        ${getCrystalStarOptions()
          .map((option) => `<option value="${option.value}" ${getStarCount(crystalSimulatorState.star) === option.value ? "selected" : ""}>${option.label}</option>`)
          .join("")}
      </select>
    </label>
  `;
...
function renderAlchemyRecipeSelector() {
  ...
  return `
    <div class="crystal-alchemy-selector">
      <label class="field crystal-field">
        <span>錬金候補選択</span>
        <select data-crystal-field="alchemyRecipeId">
          ${recipes.length === 0 ? `<option value="">候補なし</option>` : ""}
          ${recipes
            .map((recipe) => `<option value="${escapeHtml(recipe.id)}" ${selectedRecipe?.id === recipe.id ? "selected" : ""}>${escapeHtml(recipe.typeLabel)}: ${escapeHtml(recipe.effectName)}（${formatCrystalGold(recipe.unitCost)} / 回）</option>`)
            .join("")}
        </select>
      </label>
    </div>
...
function collectCrystalSimulatorFormValues() {
  if (!crystalSimulatorWrap) return;
  crystalSimulatorWrap.querySelectorAll("[data-crystal-field]").forEach((field) => {
    const key = field.dataset.crystalField;
    if (!key) return;
    if (field instanceof HTMLInputElement && field.type === "checkbox") {
      crystalSimulatorState[key] = field.checked;
      return;
    }
    ...
    if (key === "feeRate") {
      crystalSimulatorState[key] = getCrystalSimulatorFeeRate(value);
      return;
    }
    if (key === "equipmentLevel") {
      crystalSimulatorState[key] = getCrystalEquipmentLevel(value);
      return;
    }
    if (key === "star") {
      crystalSimulatorState[key] = getStarCount(value);
      return;
    }
    crystalSimulatorState[key] = parseGoldInput(value);
  });
...
function switchCrystalSimulatorTab(tabId) {
  ...
  } else if (activeCrystalSimulatorTab === "craft-alchemy") {
    isCrystalCraftMaterialCostManual = false;
    updateCraftAlchemyCost();
  }
...
function buildCrystalSimulatorResultHtml() {
  if (activeCrystalSimulatorTab === "buy") {
    const result = calculateBuyCrystal();
    return `
      <div class="crystal-result-grid">
        <p><span>結晶数</span><strong>${result.crystalCount}個</strong></p>
        <p><span>結晶売却手取り</span><strong>${formatCrystalGold(result.crystalNetSales)}</strong></p>
        <p><span>購入単価</span><strong>${formatCrystalGold(result.purchasePrice)}</strong></p>
        <p><span>利益</span><strong class="${result.profit >= 0 ? "is-positive" : "is-negative"}">${formatCrystalGold(result.profit)}</strong></p>
        <p><span>利益率</span><strong>${formatCrystalPercent(result.profitRate)}</strong></p>
        <p><span>判定</span><strong>${escapeHtml(result.judgement)}</strong></p>
      </div>
    `;
  }

  if (activeCrystalSimulatorTab === "buy-alchemy") {
    const result = calculateBuyAlchemy();
    return `
      <div class="crystal-result-grid">
        <p><span>総原価</span><strong>${formatCrystalGold(result.totalCost)}</strong></p>
        <p><span>錬金素材原価</span><strong>${formatCrystalGold(result.alchemyMaterialCost)}</strong></p>
        <p><span>結晶数</span><strong>${result.crystalCount}個</strong></p>
        <p><span>結晶化利益</span><strong class="${result.crystalProfit >= 0 ? "is-positive" : "is-negative"}">${formatCrystalGold(result.crystalProfit)}</strong></p>
        <p><span>バザー出品利益</span><strong class="${result.bazaarProfit >= 0 ? "is-positive" : "is-negative"}">${formatCrystalGold(result.bazaarProfit)}</strong></p>
        <p><span>おすすめ</span><strong>${escapeHtml(result.recommendation)}</strong></p>
        <p><span>差額</span><strong>${formatCrystalGold(result.difference)}</strong></p>
      </div>
    `;
  }

  const result = calculateCraftAlchemy();
  return `
    <div class="crystal-result-grid">
      <p><span>素材原価</span><strong>${formatCrystalGold(result.productionCost)}</strong></p>
      <p><span>錬金素材原価</span><strong>${formatCrystalGold(result.alchemyMaterialCost)}</strong></p>
      <p><span>総原価</span><strong>${formatCrystalGold(result.totalCost)}</strong></p>
      <p><span>結晶数</span><strong>${result.crystalCount}個</strong></p>
      <p><span>結晶化利益</span><strong class="${result.crystalProfit >= 0 ? "is-positive" : "is-negative"}">${formatCrystalGold(result.crystalProfit)}</strong></p>
...
function renderCrystalSimulatorResult() {
  collectCrystalSimulatorFormValues();
  const resultWrap = crystalSimulatorWrap?.querySelector("#crystalSimulatorResultWrap");
  if (resultWrap) {
    resultWrap.innerHTML = buildCrystalSimulatorResultHtml();
  }
}

function renderCrystalSimulatorFormFields() {
  if (activeCrystalSimulatorTab === "buy") {
    return `
      ${renderCrystalEquipmentSelector()}
...
function renderCrystalAutoDataInfoCard() {
  return `
    <section class="card crystal-auto-data-card" aria-label="自動反映データについて">
      <div class="crystal-auto-data-desktop">
        <h3>自動反映データについて</h3>
      ...
      <details class="crystal-auto-data-mobile">
        ...
        <ul class="crystal-auto-data-list">
          ...
          <li><strong>購入単価</strong><span>一部装備の未錬金★★ / ★★★価格を反映</span></li>
          <li><strong>素材原価</strong><span>レシピ素材と素材価格から計算</span></li>
          <li><strong>錬金原価</strong><span>初級錬金レシピから計算</span></li>
        </ul>
      </details>
    </section>
  `;
...
function renderCrystalSimulator() {
  ...
  if (activeCrystalSimulatorTab === "craft-alchemy" && !isCrystalCraftMaterialCostManual) {
    updateCraftAlchemyCost();
  }
  ...
  crystalSimulatorWrap.innerHTML = `
    ...
    <section class="card crystal-simulator-card">
...
function normalizeCraftRecord(record) {
  const totalCount = toNonNegativeInteger(record?.totalCount ?? record?.total_count);
  const legacyTotalCost = toNonNegativeNumber(record?.totalCost ?? record?.total_cost);
  const legacyTotalSales = toNonNegativeNumber(record?.totalSales ?? record?.total_sales);
  const legacyUnitSales =
    record?.unitSales !== undefined || record?.unit_sales !== undefined
      ? toNonNegativeNumber(record?.unitSales ?? record?.unit_sales)
      : totalCount > 0
        ? legacyTotalSales / totalCount
        : legacyTotalSales;
  const unitCost =
    record?.unitCost !== undefined || record?.unit_cost !== undefined
      ? toNonNegativeNumber(record?.unitCost ?? record?.unit_cost)
      : totalCount > 0
        ? legacyTotalCost / totalCount
        : legacyTotalCost;
  const star0Sales = record?.star0Sales !== undefined || record?.star0_sales !== undefined ? toNonNegativeNumber(record?.star0Sales ?? record?.star0_sales) : legacyUnitSales;
  const star1Sales = record?.star1Sales !== undefined || record?.star1_sales !== undefined ? toNonNegativeNumber(record?.star1Sales ?? record?.star1_sales) : legacyUnitSales;
  const star2Sales = record?.star2Sales !== undefined || record?.star2_sales !== undefined ? toNonNegativeNumber(record?.star2Sales ?? record?.star2_sales) : legacyUnitSales;
  const star3Sales = record?.star3Sales !== undefined || record?.star3_sales !== undefined ? toNonNegativeNumber(record?.star3Sales ?? record?.star3_sales) : legacyUnitSales;
  const startTime = normalizeCraftRecordDateTime(record?.startTime ?? record?.start_time);
  const endTime = normalizeCraftRecordDateTime(record?.endTime ?? record?.end_time);
  const durationSeconds = toNonNegativeInteger(record?.durationSeconds ?? record?.duration_seconds) || getCraftRecordDurationSecondsBetween(startTime, endTime);
  return {
...
function normalizeCraftRecordDateTime(value) {
  const text = String(value || "").trim();
  if (!text) return "";
...
function getCraftRecordDurationSecondsBetween(startTime, endTime) {
...
function getCraftRecordCurrentWorkDurationSeconds() {
  if (!craftRecordWorkStartTime) return 0;
  const endTime = craftRecordWorkEndTime || new Date().toISOString();
  return getCraftRecordDurationSecondsBetween(craftRecordWorkStartTime, endTime);
}
...
function getCraftRecordTotalCost(record) {
  return toNonNegativeInteger(record?.totalCount) * toNonNegativeNumber(record?.unitCost);
}

function getCraftRecordTotalSales(record) {
  return (
    toNonNegativeInteger(record?.star0) * toNonNegativeNumber(record?.star0Sales) +
    toNonNegativeInteger(record?.star1) * toNonNegativeNumber(record?.star1Sales) +
    toNonNegativeInteger(record?.star2) * toNonNegativeNumber(record?.star2Sales) +
    toNonNegativeInteger(record?.star3) * toNonNegativeNumber(record?.star3Sales)
  );
}

function getCraftRecordProfit(record) {
  return getCraftRecordTotalSales(record) - getCraftRecordTotalCost(record);
}

function getCraftRecordDurationSeconds(record) {
  return toNonNegativeInteger(record?.durationSeconds) || getCraftRecordDurationSecondsBetween(record?.startTime, record?.endTime);
}

function formatCraftRecordDuration(seconds) {
  const totalSeconds = toNonNegativeInteger(seconds);
  if (totalSeconds <= 0) return "-";
  const hours = Math.floor(totalSeconds / 3600);
  const minutes = Math.floor((totalSeconds % 3600) / 60);
  const restSeconds = totalSeconds % 60;
  if (hours > 0) return `${hours}時間${minutes}分${restSeconds}秒`;
  if (minutes > 0) return `${minutes}分${restSeconds}秒`;
  return `${restSeconds}秒`;
}

function formatCraftRecordSecondsPerItem(durationSeconds, totalCount) {
  const count = toNonNegativeInteger(totalCount);
  const seconds = toNonNegativeInteger(durationSeconds);
  if (seconds <= 0 || count <= 0) return "-";
  return formatCraftRecordDuration(Math.round(seconds / count));
}

function formatCraftRecordProfitRate(profit, durationSeconds, minutes) {
  const seconds = toNonNegativeInteger(durationSeconds);
  if (seconds <= 0) return "-";
  const profitValue = Number(profit);
  return formatGold(((Number.isFinite(profitValue) ? profitValue : 0) / seconds) * 60 * minutes);
}
...
function getFilteredCraftRecords() {
  ...
  return craftRecords.filter((record) => {
    if (selectedCraftRecordItemName && record.itemName !== selectedCraftRecordItemName) return false;
...
function getVisibleCraftRecordListRecords(records) {
  const dateKeyword = craftRecordListDateKeyword.trim();
  const itemKeyword = craftRecordListItemKeyword.trim().toLowerCase();
  return records
    .filter((record) => {
      if (dateKeyword && !String(record.date || "").includes(dateKeyword)) return false;
      if (itemKeyword && !String(record.itemName || "").toLowerCase().includes(itemKeyword)) return false;
      return true;
    })
    .sort((a, b) => {
      const dateCompare = String(a.date).localeCompare(String(b.date)) || String(a.createdAt).localeCompare(String(b.createdAt));
      return selectedCraftRecordListSort === "date_asc" ? dateCompare : -dateCompare;
    });
}

function getCraftRecordItemNames() {
  return Array.from(new Set(craftRecords.map((record) => record.itemName).filter(Boolean))).sort((a, b) => a.localeCompare(b, "ja"));
}

function renderCraftRecordItemFilter() {
  if (!craftRecordItemFilterSelect) return;
  const itemNames = getCraftRecordItemNames();
  if (selectedCraftRecordItemName && !itemNames.includes(selectedCraftRecordItemName)) {
    selectedCraftRecordItemName = "";
  }
  craftRecordItemFilterSelect.innerHTML = [
    `<option value="">すべて</option>`,
    ...itemNames.map((itemName) => `<option value="${escapeHtml(itemName)}">${escapeHtml(itemName)}</option>`),
  ].join("");
  craftRecordItemFilterSelect.value = selectedCraftRecordItemName;
}
...
function summarizeCraftRecords(records) {
  return records.reduce(
    (summary, record) => {
      ...
      summary.totalSales += getCraftRecordTotalSales(record);
      summary.totalCost += getCraftRecordTotalCost(record);
      summary.profit += getCraftRecordProfit(record);
      summary.durationSeconds += getCraftRecordDurationSeconds(record);
      return summary;
    },
    { totalCount: 0, star0: 0, star1: 0, star2: 0, star3: 0, totalSales: 0, totalCost: 0, profit: 0, durationSeconds: 0 }
  );
}
...
function formatCraftRecordTimeLabel(value) {
  const date = new Date(value);
  if (!Number.isFinite(date.getTime())) return "-";
  return date.toLocaleTimeString("ja-JP", { hour: "2-digit", minute: "2-digit", second: "2-digit" });
}

function getCraftRecordFormRecord() {
  return normalizeCraftRecord({
    ...
    unitCost: craftRecordCostInput?.value,
    star0Sales: craftRecordStar0SalesInput?.value,
    star1Sales: craftRecordStar1SalesInput?.value,
    star2Sales: craftRecordStar2SalesInput?.value,
    star3Sales: craftRecordStar3SalesInput?.value,
    startTime: craftRecordWorkStartTime,
    endTime: craftRecordWorkEndTime,
    durationSeconds: getCraftRecordCurrentWorkDurationSeconds(),
    memo: craftRecordMemoInput?.value,
  });
}

function updateCraftRecordWorkTimeStatus() {
  if (!craftRecordWorkTimeStatus) return;
  const durationSeconds = getCraftRecordCurrentWorkDurationSeconds();
  const formRecord = getCraftRecordFormRecord();
  const profit = getCraftRecordProfit(formRecord);
  const startLabel = craftRecordWorkStartTime ? `開始 ${formatCraftRecordTimeLabel(craftRecordWorkStartTime)}` : "開始 -";
  const endLabel = craftRecordWorkEndTime ? `終了 ${formatCraftRecordTimeLabel(craftRecordWorkEndTime)}` : "終了 -";
  craftRecordWorkTimeStatus.textContent = `作業時間：${formatCraftRecordDuration(durationSeconds)} / 1個あたり：${formatCraftRecordSecondsPerItem(durationSeconds, formRecord.totalCount)} / 30分利益：${formatCraftRecordProfitRate(profit, durationSeconds, 30)} / 時給換算：${formatCraftRecordProfitRate(profit, durationSeconds, 60)}（${startLabel} / ${endLabel}）`;
}

function setCraftRecordTimerActive(isActive) {
  if (craftRecordTimerIntervalId !== null) {
    clearInterval(craftRecordTimerIntervalId);
    craftRecordTimerIntervalId = null;
  }
  if (isActive) {
    craftRecordTimerIntervalId = window.setInterval(updateCraftRecordWorkTimeStatus, 1000);
  }
}

function resetCraftRecordWorkState() {
  craftRecordWorkStartTime = "";
  craftRecordWorkEndTime = "";
  setCraftRecordTimerActive(false);
  updateCraftRecordWorkTimeStatus();
}

function setCraftRecordInputValue(input, value) {
  if (!input) return;
  input.value = String(Math.max(0, toNonNegativeInteger(value)));
}

function incrementCraftRecordStar(star) {
  const starInputMap = {
    0: craftRecordStar0Input,
    1: craftRecordStar1Input,
    2: craftRecordStar2Input,
    3: craftRecordStar3Input,
  };
  const input = starInputMap[star];
  if (!input || !craftRecordTotalInput) return;
  setCraftRecordInputValue(input, toNonNegativeInteger(input.value) + 1);
  setCraftRecordInputValue(craftRecordTotalInput, toNonNegativeInteger(craftRecordTotalInput.value) + 1);
  craftRecordCountHistory.push(star);
  updateCraftRecordWarning();
  updateCraftRecordWorkTimeStatus();
}

function undoCraftRecordCount() {
  const star = craftRecordCountHistory.pop();
  ...
  const input = starInputMap[star];
  if (input) setCraftRecordInputValue(input, toNonNegativeInteger(input.value) - 1);
  if (craftRecordTotalInput) setCraftRecordInputValue(craftRecordTotalInput, toNonNegativeInteger(craftRecordTotalInput.value) - 1);
  updateCraftRecordWarning();
  updateCraftRecordWorkTimeStatus();
}

function resetCraftRecordCountInputs() {
  craftRecordCountHistory = [];
  [craftRecordTotalInput, craftRecordStar0Input, craftRecordStar1Input, craftRecordStar2Input, craftRecordStar3Input].forEach((input) => {
...
function getTodayCraftRecords() {
  const today = formatDateAsIsoText(new Date());
  return craftRecords.filter((record) => record.date === today && (!selectedCraftRecordItemName || record.itemName === selectedCraftRecordItemName));
}

function renderCraftRecordTodaySummary() {
  if (!craftRecordTodaySummaryWrap) return;
  const summary = summarizeCraftRecords(getTodayCraftRecords());
  craftRecordTodaySummaryWrap.innerHTML = `
    <section class="card craft-record-today-card">
      <h3>今日の集計</h3>
      <div class="craft-record-summary-grid">
        <section class="card craft-record-summary-card is-primary"><span>今日の作成数</span><strong>${summary.totalCount.toLocaleString("ja-JP")}</strong></section>
        <section class="card craft-record-summary-card is-primary"><span>今日の星3率</span><strong>${formatPercent(summary.star3, summary.totalCount)}</strong></section>
        <section class="card craft-record-summary-card is-primary"><span>今日の総利益</span><strong>${formatGold(summary.profit)}</strong></section>
        <section class="card craft-record-summary-card"><span>今日の総売上</span><strong>${formatGold(summary.totalSales)}</strong></section>
        <section class="card craft-record-summary-card"><span>今日の総原価</span><strong>${formatGold(summary.totalCost)}</strong></section>
        <section class="card craft-record-summary-card"><span>今日の作業時間</span><strong>${formatCraftRecordDuration(summary.durationSeconds)}</strong></section>
        <section class="card craft-record-summary-card"><span>今日の30分利益</span><strong>${formatCraftRecordProfitRate(summary.profit, summary.durationSeconds, 30)}</strong></section>
        <section class="card craft-record-summary-card"><span>今日の時給換算</span><strong>${formatCraftRecordProfitRate(summary.profit, summary.durationSeconds, 60)}</strong></section>
        <section class="card craft-record-summary-card"><span>今日の1個あたり</span><strong>${formatCraftRecordSecondsPerItem(summary.durationSeconds, summary.totalCount)}</strong></section>
      </div>
    </section>
  `;
}

function getCraftRecordVarianceLabel(summary, baselineSummary) {
  if (!selectedCraftRecordItemName || !baselineSummary || summary.totalCount <= 0 || baselineSummary.totalCount <= 0) return "";
  const dailyRate = summary.star3 / summary.totalCount;
  const baselineRate = baselineSummary.star3 / baselineSummary.totalCount;
  const diff = (dailyRate - baselineRate) * 100;
  if (diff >= 5) return `<span class="craft-record-variance is-up">上振れ +${diff.toFixed(1)}%</span>`;
  if (diff <= -5) return `<span class="craft-record-variance is-down">下振れ ${diff.toFixed(1)}%</span>`;
  return `<span class="craft-record-variance">平均付近</span>`;
}

function renderCraftRecordDailySummary(records) {
  if (!craftRecordDailySummaryWrap) return;
  ...
  const baselineSummary = selectedCraftRecordItemName
    ? summarizeCraftRecords(craftRecords.filter((record) => record.itemName === selectedCraftRecordItemName))
    : null;
  const rows = Array.from(groups.entries())
    .sort((a, b) => String(b[0]).localeCompare(String(a[0])))
    .map(([date, dateRecords]) => {
      const summary = summarizeCraftRecords(dateRecords);
      return `
        <article class="card craft-record-daily-card">
          <div class="craft-record-daily-header"><h4>${escapeHtml(date)}</h4>${getCraftRecordVarianceLabel(summary, baselineSummary)}</div>
          <div class="craft-record-card-stats">
            <span>作成 ${summary.totalCount.toLocaleString("ja-JP")}</span>
            <span>星3率 ${formatPercent(summary.star3, summary.totalCount)}</span>
            <span>星2率 ${formatPercent(summary.star2, summary.totalCount)}</span>
            <span>星1率 ${formatPercent(summary.star1, summary.totalCount)}</span>
            <span>星0率 ${formatPercent(summary.star0, summary.totalCount)}</span>
          </div>
          <div class="craft-record-card-money">
            <strong>純利益 ${formatGold(summary.profit)}</strong>
            <span>作業時間 ${formatCraftRecordDuration(summary.durationSeconds)}</span>
            <span>30分利益 ${formatCraftRecordProfitRate(summary.profit, summary.durationSeconds, 30)}</span>
            <span>時給換算 ${formatCraftRecordProfitRate(summary.profit, summary.durationSeconds, 60)}</span>
          </div>
        </article>
      `;
    });
  craftRecordDailySummaryWrap.innerHTML = `
    <section class="craft-record-daily-summary">
      <h3>日別集計</h3>
      ${rows.length > 0 ? `<div class="craft-record-daily-list">${rows.join("")}</div>` : `<p class="card craft-record-empty">この条件の日別集計はまだありません。</p>`}
    </section>
  `;
}
...
function buildCraftRecordFromForm() {
  const startTime = craftRecordWorkStartTime;
  const endTime = craftRecordWorkEndTime || (craftRecordWorkStartTime ? new Date().toISOString() : "");
  return normalizeCraftRecord({
    ...
    startTime,
    endTime,
    durationSeconds: getCraftRecordDurationSecondsBetween(startTime, endTime),
    memo: craftRecordMemoInput?.value,
  });
}
...
function clearCraftRecordForm() {
  ...
  [
    ...
    craftRecordCostInput,
    craftRecordStar0SalesInput,
    craftRecordStar1SalesInput,
    craftRecordStar2SalesInput,
    craftRecordStar3SalesInput,
  ].forEach((input) => {
    if (input) input.value = "0";
  });
  if (craftRecordMemoInput) craftRecordMemoInput.value = "";
  craftRecordCountHistory = [];
  resetCraftRecordWorkState();
  updateCraftRecordWarning();
}
...
function renderCraftRecords() {
  ...
  renderCraftRecordItemFilter();
  updateCraftRecordWarning();
  updateCraftRecordWorkTimeStatus();
  const filteredRecords = getFilteredCraftRecords();
  ...
  craftRecordSummaryWrap.innerHTML = `
    <div class="craft-record-summary-grid">
      ...
      <section class="card craft-record-summary-card"><span>作業時間</span><strong>${formatCraftRecordDuration(summary.durationSeconds)}</strong></section>
      <section class="card craft-record-summary-card"><span>30分利益</span><strong>${formatCraftRecordProfitRate(summary.profit, summary.durationSeconds, 30)}</strong></section>
      <section class="card craft-record-summary-card"><span>時給換算</span><strong>${formatCraftRecordProfitRate(summary.profit, summary.durationSeconds, 60)}</strong></section>
      <section class="card craft-record-summary-card"><span>1個あたり作成時間</span><strong>${formatCraftRecordSecondsPerItem(summary.durationSeconds, summary.totalCount)}</strong></section>
    </div>
  `;
  renderCraftRecordTodaySummary();
  renderCraftRecordDailySummary(filteredRecords);
  if (craftRecordListDateFilterInput && craftRecordListDateFilterInput.value !== craftRecordListDateKeyword) {
    craftRecordListDateFilterInput.value = craftRecordListDateKeyword;
  }
  if (craftRecordListItemFilterInput && craftRecordListItemFilterInput.value !== craftRecordListItemKeyword) {
    craftRecordListItemFilterInput.value = craftRecordListItemKeyword;
  }
  if (craftRecordListSortSelect && craftRecordListSortSelect.value !== selectedCraftRecordListSort) {
    craftRecordListSortSelect.value = selectedCraftRecordListSort;
  }
  const visibleListRecords = getVisibleCraftRecordListRecords(filteredRecords);
  if (visibleListRecords.length === 0) {
    craftRecordListWrap.innerHTML = `<p class="card craft-record-empty">この条件の職人記録はまだありません。</p>`;
    return;
  }
    ...
    <div class="craft-record-list">
      ${visibleListRecords
        .map((record) => {
          ...
          const totalCost = getCraftRecordTotalCost(record);
          const totalSales = getCraftRecordTotalSales(record);
          const durationSeconds = getCraftRecordDurationSeconds(record);
          return `
            <article class="card craft-record-card${hasMismatch ? " has-warning" : ""}">
              ...
              <div class="craft-record-card-money">
                <span>原価 ${formatGold(record.unitCost)}</span>
                <span>星0売上 ${formatGold(record.star0Sales)}</span>
                <span>星1売上 ${formatGold(record.star1Sales)}</span>
                <span>星2売上 ${formatGold(record.star2Sales)}</span>
                <span>星3売上 ${formatGold(record.star3Sales)}</span>
                <span>総原価 ${formatGold(totalCost)}</span>
                <span>総売上 ${formatGold(totalSales)}</span>
                <strong>総利益 ${formatGold(getCraftRecordProfit(record))}</strong>
                <span>作業時間 ${formatCraftRecordDuration(durationSeconds)}</span>
                <span>30分利益 ${formatCraftRecordProfitRate(getCraftRecordProfit(record), durationSeconds, 30)}</span>
                <span>時給換算 ${formatCraftRecordProfitRate(getCraftRecordProfit(record), durationSeconds, 60)}</span>
                <span>1個あたり ${formatCraftRecordSecondsPerItem(durationSeconds, record.totalCount)}</span>
              </div>
...
function exportCraftRecordsCsv() {
  const headers = ["id", "date", "item_name", "total_count", "star0", "star1", "star2", "star3", "unit_cost", "star0_sales", "star1_sales", "star2_sales", "star3_sales", "start_time", "end_time", "duration_seconds", "memo", "created_at"];
  ...
  craftRecords.forEach((record) => {
    lines.push(
      [
        ...
        record.startTime,
        record.endTime,
        getCraftRecordDurationSeconds(record),
        record.memo,
        record.createdAt,
      ]
        .map(escapeCsvValue)
  ...
  return lines
    ...
    .map((row) =>
      normalizeCraftRecord({
        ...
        unitCost: row[getIndex("unit_cost")],
        star0Sales: row[getIndex("star0_sales")],
        star1Sales: row[getIndex("star1_sales")],
        star2Sales: row[getIndex("star2_sales")],
        star3Sales: row[getIndex("star3_sales")],
        unitSales: row[getIndex("unit_sales")],
        totalCost: row[getIndex("total_cost")],
        totalSales: row[getIndex("total_sales")],
        startTime: row[getIndex("start_time")],
        endTime: row[getIndex("end_time")],
        durationSeconds: row[getIndex("duration_seconds")],
...
function switchTab(target) {
  ...
  if (normalizedTarget === "present-codes") {
    renderPresentCodes();
  } else if (normalizedTarget === "crystal-simulator") {
    renderCrystalSimulator();
...
function getDefaultTabDocumentDescription(tabId) {
  switch (String(tabId || "").trim()) {
    ...
    case "crystal-simulator":
      return "装備購入・錬金・制作から、結晶化とバザー出品の利益を比較できるDQ10ツールです。";
    case "steal-farming":
...
if (crystalSimulatorWrap) {
  ...
  crystalSimulatorWrap.addEventListener("input", (event) => {
    ...
    if (String(event.target.dataset.crystalField || "") === "materialCost") {
      isCrystalCraftMaterialCostManual = true;
    }
    renderCrystalSimulatorResult();
  });

  crystalSimulatorWrap.addEventListener("change", (event) => {
    const target = event.target;
    if (!(target instanceof HTMLElement)) return;
  ...
  crystalSimulatorWrap.addEventListener("click", (event) => {
    const target = event.target;
    if (!(target instanceof Element)) return;

    const tabButton = target.closest("[data-crystal-tab]");
    if (tabButton instanceof HTMLElement) {
      switchCrystalSimulatorTab(String(tabButton.dataset.crystalTab || ""));
      return;
    }

    const alchemyButton = target.closest("[data-crystal-alchemy-id]");
    if (alchemyButton instanceof HTMLElement) {
      collectCrystalSimulatorFormValues();
      crystalSimulatorState.alchemyRecipeId = String(alchemyButton.dataset.crystalAlchemyId || "");
      const recipe = getSelectedAlchemyRecipe();
      crystalSimulatorState.alchemyEffectName = recipe?.effectName || "";
      crystalSimulatorState.alchemyUnitCost = Number(recipe?.unitCost || 0);
      resetCrystalCountToEstimate();
    ...
    const materialBreakdownToggle = target.closest("[data-crystal-material-breakdown-toggle]");
    if (materialBreakdownToggle instanceof HTMLElement) {
      collectCrystalSimulatorFormValues();
      isCrystalMaterialBreakdownExpanded = !isCrystalMaterialBreakdownExpanded;
      renderCrystalSimulator();
      return;
    }
    ...
    const relatedButton = target.closest("[data-crystal-related-tab]");
    if (relatedButton instanceof HTMLElement) {
      switchTab(String(relatedButton.dataset.crystalRelatedTab || ""));
    }
  });
}
...
craftRecordCountButtons.forEach((button) => {
  button.addEventListener("click", () => {
    const star = toNonNegativeInteger(button.getAttribute("data-craft-record-count-star"));
    incrementCraftRecordStar(star);
  });
});

if (craftRecordUndoCountButton) {
  craftRecordUndoCountButton.addEventListener("click", undoCraftRecordCount);
}

if (craftRecordResetCountButton) {
  craftRecordResetCountButton.addEventListener("click", resetCraftRecordCountInputs);
}

if (craftRecordWorkStartButton) {
  craftRecordWorkStartButton.addEventListener("click", () => {
    craftRecordWorkStartTime = new Date().toISOString();
    craftRecordWorkEndTime = "";
    if (craftRecordDateInput && !craftRecordDateInput.value) craftRecordDateInput.value = formatDateAsIsoText(new Date());
    setCraftRecordTimerActive(true);
    updateCraftRecordWorkTimeStatus();
  });
}

if (craftRecordWorkEndButton) {
  craftRecordWorkEndButton.addEventListener("click", () => {
    if (!craftRecordWorkStartTime) {
      if (craftRecordWarning) craftRecordWarning.textContent = "先に作業開始を押してください。";
      return;
    }
    craftRecordWorkEndTime = new Date().toISOString();
    setCraftRecordTimerActive(false);
    updateCraftRecordWorkTimeStatus();
  });
}
...
if (craftRecordItemFilterSelect) {
  craftRecordItemFilterSelect.addEventListener("change", () => {
    selectedCraftRecordItemName = String(craftRecordItemFilterSelect.value || "");
    renderCraftRecords();
  });
}

if (craftRecordListDateFilterInput) {
  craftRecordListDateFilterInput.addEventListener("input", () => {
    craftRecordListDateKeyword = String(craftRecordListDateFilterInput.value || "").trim();
    renderCraftRecords();
  });
}

if (craftRecordListItemFilterInput) {
  craftRecordListItemFilterInput.addEventListener("input", () => {
    craftRecordListItemKeyword = String(craftRecordListItemFilterInput.value || "").trim();
    renderCraftRecords();
  });
}

if (craftRecordListSortSelect) {
  craftRecordListSortSelect.addEventListener("change", () => {
    selectedCraftRecordListSort = String(craftRecordListSortSelect.value || "date_desc");
    renderCraftRecords();
  });
}
