...
- 主な使用項目:
  ...
  - `comment`
- 補足:
  - 素材カード右側の「更新時刻」は `data/last-updated.json` の `bazaarCsvUpdatedAt` を全行共通で表示
  - `last-updated.json` が読めない/値が空の場合、更新時刻は `-` を表示
  - カード右側の更新時刻表示では `bazaar_prices.csv` の `updated_at` は使用しない
...
- 用途:
  - バザー価格一覧カード右側の「更新時刻」表示（全素材共通）
- 主な使用項目:
  - `bazaarCsvUpdatedAt`
- 更新方法:
  - **JSON更新（手動差し替え）**
  - `data/last-updated.json` を更新すると、次回読込時に反映
...
- `data/last-updated.json` : バザー更新時刻（全素材共通表示）
...
- 2026-04-03: ヘッダーの全体更新時刻表示を廃止し、`data/last-updated.json` の `bazaarCsvUpdatedAt` をバザー素材カード右側の更新時刻として全行共通表示する仕様へ変更（JSON読込失敗時は `-` 表示）。
